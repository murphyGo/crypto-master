...
# ``ActivityEvent`` / ``ActivityEventType`` moved to the pure
# ``src.runtime.activity_events`` module (CAH-13 / LAYER-F4). They are
# imported above and re-exported here so every existing
# ``from src.runtime.activity_log import ActivityEvent`` path keeps
# resolving — this module is the IO adapter (writer/reader) and pure
# consumers should prefer the ``activity_events`` source module.
