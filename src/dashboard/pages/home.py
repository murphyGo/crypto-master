"""Home command-center page (CAH-09 / DASH-F6 split).

Extracted verbatim from ``dashboard/app.py``: the Home command-center
read-model, its pure builders, and the two render functions
(``render_command_center_status`` + ``render_home``). ``app.py`` is left as
the pure navigation chassis and registers ``render_home`` like every other
page.

Import-cycle note (CAH-09): ``page_for_key`` lives in the ``app.py`` chassis
and is needed only by :func:`render_command_center_links` for drill-through
``st.page_link``s. To avoid a ``home`` ↔ ``app`` cycle it is imported lazily
inside that function, mirroring the existing lazy ``proposals_page`` import in
``render_home``. The sibling page imports (``engine``/``trading``/``feedback``)
are safe — those modules never import this one.
...
def render_command_center_links(
    ...
    *,
    ...
    # CAH-09 import-cycle care: ``page_for_key`` lives in the ``app.py``
    # chassis (which imports THIS module to register ``render_home``).
    # Import it lazily here so the module graph stays acyclic.
    from src.dashboard.app import page_for_key
