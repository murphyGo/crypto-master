...
**Suggested Resolution:**
Sequential cycles — (b) opt-in global caps shipped 2026-05-24; (c-1) stateless kill-switch gates shipped 2026-05-24; (c-2) stateful daily-loss kill switch shipped 2026-05-24, completing DEBT-068(c). Next (c-arb) cap arbitration or (d) operator freeze, then (e) stale actions, (f) dashboard, and (g) event type. (d) operator freeze reload and (h) runtime-safety-score integration are smaller and can bundle with (f).
