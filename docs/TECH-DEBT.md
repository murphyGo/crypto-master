...
- **(c-arb) `cap_resolution=lowest_priority_loses` arbitration for global `(symbol, side)` caps** — DEBT-068(b) shipped `_global_aggregate_cap_gate` with `first_come_first_serve` v1 only; `lowest_priority_loses` arbitration is deferred. The DEBT-068(b) construction-plan step attributed this deferral to "DEBT-068(c)", but (c) is kill switches only, so this is split out as its own sub-bullet (2026-05-24) to keep the deferral ticketed. Separate slice from the kill switches; not to be bundled into (c). Sequencing is the lead's call.
...
- **(f) Dashboard exposure panel + operator-freeze toggle WRITE side** — per-account + global aggregate views, kill-switch state indicator, and the operator-freeze toggle UI. The runtime READ side of the freeze shipped under DEBT-068(d) on 2026-05-24 (`config/runtime_flags.yaml` + `read_trading_freeze` reader + per-cycle gate); what remains here is the dashboard WRITE side — the toggle UI that persists `trading_freeze` back to `config/runtime_flags.yaml` — plus the freeze-state indicator. Match runtime-reconciliation banner color pattern.
...
**Suggested Resolution:**
Sequential cycles — (b) opt-in global caps shipped 2026-05-24; (c-1) stateless kill-switch gates shipped 2026-05-24; (c-2) stateful daily-loss kill switch shipped 2026-05-24, completing DEBT-068(c); (d) operator-freeze RUNTIME READ side shipped 2026-05-24; (e) stale `auto_close` / `alert_only` actions shipped 2026-05-24; (g) dedicated `RISK_CAP_ADVISORY` / `RISK_KILL_SWITCH_TRIPPED` event types shipped 2026-05-24 (event-type migration only). Next (c-arb) cap arbitration (recommended — closes the last open-cap v1-arbitration gap left by (b)), then (f) dashboard exposure panel + operator-freeze toggle WRITE side (the natural home for the (d) toggle UI, the (e) stale-event surfacing, and the (g-note-dashboard-undercount) "Rejected"-column rebase). (h) runtime-safety-score integration is smaller and can bundle with (f).
