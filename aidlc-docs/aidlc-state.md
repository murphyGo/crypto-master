...
| `consistency-hardening` | In progress | P1 slice "Legacy exchange config mode/credential alignment" shipped (2026-05-09): runtime testnet flag now drives `BinanceConfig` / `BybitConfig` credential selection so ccxt sandbox URL and keys stay aligned when the factory overrides the env-driven testnet default | Continue with the remaining P1 slices: `.py` strategy promotion integrity, sub-account runtime failure isolation, notification backend failure visibility, dashboard aggregate/scope consistency |
